...
$(function(){
  ...
  var block = hackwedata.feed.entry[2].content.IntervalBlock;
  ...
  function updateGraph() {
    var blockTemp = [];
    var randBlockData = [];
    var n = get_random_number(timeInterval, block.length-timeInterval-1);
    for(var k = 0; k < timeInterval ; k++) {
      ...
      for ( var j = 0; j < Math.min(block[k].IntervalReading.length,block[n+k].IntervalReading.length); ++j ){
        if ( timeInterval == 1 ) {
